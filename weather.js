...
function parseCurrentWeather({current_weather, daily}){
    ...
    const {
        // This is the same as saying const maxTemp = daily.temperature_2m_max[0], where you get the first value in the array
    ...
    return {
        ...
        // Rounding precip to the nearest 100th
...
function parseDailyWeather({daily}) {
    return daily.time.map((time, index) => {
        return {
            // API returns the time in seconds, JS expects time in ms, thats why I multiplied by 1000
            timestamp: time * 1000,
...
function parseHourlyWeather({hourly, current_weather}) { 
    ...
    // the filter ensures that we only display weather data for after the current time
    // there's no point displaying weather info for the hours of the day that have already passed
}